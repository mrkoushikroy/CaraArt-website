...
$cart_table = mysqli_query($dbconn,"SELECT sum(total) FROM order_details WHERE user_id='$user_id' AND order_id=''") or die(mysqli_error());
       $cart_count = mysqli_num_rows($cart_table); 
        ...
        while ($cart_row = mysqli_fetch_array($cart_table)) {
           ...
           $ord = "INSERT INTO order_user (user_id, track_num, firstname, middlename, lastname, email, contact, shipping_add, order_date, status, totalprice, tax) VALUES ('$user_id','$track_num','$firstname','$middlename','$lastname','$email','$contact','$ship_add','$date','Pending','$total','$tax')";  
           $result = mysqli_query($dbconn,$ord);
           echo $result;
